...
in
{
  ...
  programs.dank-material-shell = {
    ...
    settings = {
      ...
      barConfigs = [
        {
          ...
          centerWidgets = [ "clock" ];
          # No clipboard widget (no clipboard manager here) and no battery
          # widget (all hosts are desktops).
          rightWidgets = [
            "privacyIndicator"
            "idleInhibitor"
            "music"
      ...
      # Reproduces the retired hypridle timings: lock at 300s, monitors off at 900s.
      acLockTimeout = 300;
      acPostLockMonitorTimeout = 600;
      batteryLockTimeout = 300;
      batteryPostLockMonitorTimeout = 600;
    ...
    # wallpaperPath is left unset so Hyprland's misc.background_color shows through.
    session = {
      configVersion = 3;
      isLightMode = false;
